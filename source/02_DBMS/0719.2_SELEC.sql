...
SELECT * FROM DEPT; --DEPT테이블의 모든 열(필드), 모든행
SELECT * FROM SALGRADE; --SALGRADE테이블의 모든 열(필드), 모든 행

-- 2. 특정 열만 출력
DESC EMP; 
            -- EMP테이블의 구조
SELECT EMPNO, ENAME, SAL, JOB FROM EMP; --EMP테이블의 EMPNO, ENAME, SAL, JOB필드의 모든 행
...
SELECT EMPNO 사번, ENAME ENAME, SAL FROM EMP WHERE SAL != 3000; -- 다르다
SELECT EMPNO 사번, ENAME ENAME, SAL FROM EMP WHERE SAL ^= 3000; -- 다르다
SELECT EMPNO 사번, ENAME ENAME, SAL FROM EMP WHERE SAL <> 3000; -- 다르다
SELECT * FROM EMP WHERE SAL<3000; --
    -- 비교연산자는 숫자형(NUMBER), 문자(VARCHAR2), DATE 모두 가능
    -- EX1. 사원이름(ENAME)이 'A', 'B', 'C'로 시작하는 사원의 모든 필드 출력
        -- A < AA < AAA < AAAA < AAAA...A < B < BA < BAA..A < BB  아스키코드로 비교
    SELECT * FROM EMP WHERE ENAME < 'D';
    -- EX2. 81년도이전에 입사한 사원의 모든 필드
    SELECT * FROM EMP WHERE HIREDATE < '81/01/01';
    -- EX.3 부서번호(DEPTNO)가 10번 부서인 사원의 모든 필드
    SELECT * FROM EMP WHERE DEPTNO = 10;
    -- EX.4 이름(ENAME)이 FORD인 직원의 사번(EMPNO), 이름(ENAME), 상사의 사번(MGR)을 출력
...
-- 4. 조건절의 논리연산자 : AND, OR, NOT
    ...
    SELECT ENAME, SAL, SAL*12 ANNUALSAL --(3)
        FROM EMP --(1)
        WHERE SAL*12>=2400; -- WHERE절에는 필드의 별칭 사용 불가 --(2)
    ...
    SELECT * FROM EMP WHERE DEPTNO = 10 OR JOB = 'MANAGER'; -- 데이터는 대소문자 구분
...
SELECT EMPNO, ENAME, SAL 현재월금, SAL*1.1 올릴월급 FROM EMP;
    ...
    -- NVL(NULL일수도 있는 필드명, 대체값)을 이용 : 필드명과 대체값은 타입(문자, 숫자, 날짜)이 일치해야함
    SELECT ENAME, SAL, COMM, SAL*12+NVL(COMM, 0) ANNUALSAL FROM EMP; -- COMM이 NULL이면 0으로대체
...
SELECT EMPNO 사번, ENAME 이름, JOB 업무, HIREDATE 입사일 FROM EMP WHERE HIREDATE>'81/02/01';
 -- 날짜 셋팅이 RR/MM/DD가 아닐 수 있을 경우를 고려
SELECT EMPNO 사번, ENAME 이름, JOB 업무, HIREDATE 입사일 FROM EMP WHERE TO_CHAR(HIREDATE, 'RR/MM/DD') > '81/02/01';
...
SELECT EMPNO 사번, ENAME 사람, SAL 급여 FROM EMP WHERE SAL >= 1500 AND SAL <= 3000;
--10. 부서코드가 10번이거나 30인 사람의 사번, 이름, 업무, 부서코드 출력
SELECT EMPNO 사번, ENAME 이름, JOB 업무, DEPTNO 부서코드 FROM EMP WHERE DEPTNO=10 OR DEPTNO =30;
--11. 업무가 SALESMAN이거나 급여가 3000이상인 사람의 사번, 이름, 업무, 부서코드 출력
SELECT EMPNO 사번, ENAME 이름, SAL 급여,  JOB 업무, DEPTNO 부서코드 FROM EMP WHERE SAL>=3000 OR JOB = 'SALESMAN';
--12. 급여가 2500이상이고 업무가 MANAGER인 사람의 사번, 이름, 업무, 급여 출력
SELECT EMPNO 사번, ENAME 이름, JOB 업무, SAL 급여 FROM EMP WHERE JOB = 'MANAGER' AND SAL>=2500;
--13.“ename은 XXX 업무이고 연봉은 XX다” 스타일로 모두 출력(연봉은 SAL*12+COMM)
SELECT ENAME || '은(는)' || JOB || '업무이고 연봉은' ||(SAL*12+NVL(COMM, 0)) MESSAGE FROM EMP; 
...
-- 8. SQL연산자 (BETWEEN A AND B, IN (A, B), LIKE 패턴, IS NULL)
  ...
  SELECT * FROM EMP WHERE ENAME LIKE '%S';
   --ex. SAL이 5로 끝나는 사원의 이름
  SELECT * FROM EMP WHERE SAL LIKE '%5';
   -- ex. 82년도에 입사한 사원의 모든 필드
  SELECT * FROM EMP WHERE HIREDATE LIKE '82/%';
  SELECT * FROM EMP WHERE TO_CHAR(HIREDATE, 'RR/MM/DD') LIKE '82/__/__';
   -- ex. 1월에 입사한 사원의 모든 필드
  SELECT * FROM EMP WHERE TO_CHAR(HIREDATE, 'RR/MM/DD') LIKE '__/01/__'; -- 퍼센트 언더바 상관없음
  
  -- (4) IS NULL (NULL 인지를 검색할 때)
   -- ex. 상여금이 없는 사원(COMM이 NULL, 0)의 모든 필드
...
SELECT ENAME, SAL, HIREDATE FROM EMP ORDER BY SAL DESC; -- 금여 내림차순 정렬 
