-- [VII] DDL, DML, DCL
--SQL = DDL(테이블 생성, 테이블 삭제, 테이블 구조 변경, 테이블의 모든데이터 제거) + 
--      DML(SELECT, INSERT, UPDATE, DELETE) + 
--      DCL(사용자계정 생성, 사용자에게 권한부여, 권한박탈, 사용자계정 삭제, 트랜젝션명령어)

-- ★ ★ ★ DDL ★ ★ ★
-- 1. 테이블 생성(CREATE TABLE 테이블병 ....) : 테이블 구조를 정의
CREATE TABLE BOOK (
...
CREATE TABLE BOOK(
    ...
    -- DEPT01 : DEPTNO(수2:PK), NAME(문자14), LOC(문자13)
    CREATE TABLE DEPT01 ( 
    ...
    SELECT * FROM DEPT01;
    -- EMP01 : EMPNO(숫자4:PK주키), ENAME(문자10), SAL(숫7,2), DEPTNO(숫2:FK외래키)
    CREATE TABLE EMP01 (
    ...
    CREATE TABLE EMP04
        AS
        SELECT * FROM EMP WHERE DEPTNO=10; -- EMP에서 10번 부서의 데이터만 추출해서 EML04
    SELECT * FROM EMP04;
...
-- (2) 필드수정 (MODIFY)
ALTER TABLE EMP03 MODIFY (EMPNO VARCHAR2(4)); -- 숫자 데이터가 들어있어 숫자로만 변경 가능(숫자보다 큰 사이즈로만 가능)
...
DROP TABLE EMP01;
SELECT * FROM EMP01;
DROP TABLE DETP; -- EMP테이블에서 DEPT의 DEPTNO를 참조하는 경우 EMP테이블을 삭제한 후 DEPT를 삭제
...
-- 6. 데이터 딕셔너리(데이터베이스 자원을 효율적으로 관리하기 워한 시스템 테이블 : 접근 불가)
--          VS
--    데이터 딕셔너리 뷰(접근가능)의 종류
-- (1) USER_XXX : 현 계정에서 고유하고 있는 객체(테이블, 제약조건, 뷰, 인덱스)
SELECT * FROM USER_TABLES;
SELECT * FROM USER_CONSTRAINTS;
SELECT * FROM USER_VIEWS;
SELECT * FROM USER_INDEXES;
-- (2) ALL_XXX : 현 계정에서 접근가능한 객체(테이블, 제약족건, 뷰, 인덱스)
SELECT * FROM ALL_TABLES;
SELECT * FROM ALL_CONSTRAINTS;
SELECT * FROM ALL_VIEWS;
SELECT * FROM ALL_INDEXES;
-- (3) DBA_XXX : DBA권한에서만 접근 가능 DBMS 의 모든 객체(테이블, 제약조건, 뷰, 인덱스)
...
SELECT * FROM DEPT01;
INSERT INTO DEPT01 VALUES (50, 'ACCOUNTINT', 'SEOUL');
...
CREATE TABLE EMP01 AS SELECT EMPNO, ENAME , SAL, HIREDATE, DEPTNO FROM EMP;
    ...
    ROLLBACK;
    -- ex. SCOTT사원의 부서번호 20, 직급을 MANAGER로 수정
...
SELECT S.sNO 학번, S.sNAME 이름, sSCORE 점수, S.mCODE 학과코드, mNAME 학과명, mOFFICE 학과사무실
...
SELECT ID, NAME, USERID, TO_CHAR(SALARY, '999,999,999.99')
    FROM MY_DATA;
COMMIT;
