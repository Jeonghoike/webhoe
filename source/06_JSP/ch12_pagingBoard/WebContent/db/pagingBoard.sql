...
CREATE TABLE BOARD(
    ...
    bCONTENT VARCHAR2(1000),            -- 글본문 (VARCHAR2(4000BYTE)가 넘어가면 CLOB)
    bEMAIL   VARCHAR2(50),              -- 메일주소
    bHIT     NUMBER(6) DEFAULT 0 NOT NULL, -- 글 조회수
    bPW      VARCHAR2(50) NOT NULL,     -- 글 삭제시 쓸 비밀번호
    bGROUP   NUMBER(6)    NOT NULL,     -- 글 그룹(원글의 경우 글번호/답변글인 경우 원글의BGROUP로)
...
SELECT * FROM BOARD ORDER BY BGROUP DESC; -- 출력기준
SELECT ROWNUM RN, A.*
...
INSERT INTO BOARD (BID, BNAME, BTITLE, BCONTENT, BEMAIL, BPW, BGROUP, BSTEP, BINDENT, BIP)
    VALUES (BOARD_SEQ.NEXTVAL, '김', '제목3', '냉무', 'K@K.COM', '111', BOARD_SEQ.CURRVAL, 0, 0, '192.0.0.1');
-- 4. BID로 조회수 1 올리기 (글상세보기시 필요)
UPDATE BOARD SET BHIT = BHIT + 1 WHERE BID=1;
-- 5. BID로 DTO 가져오기(글수정FORM, 답변글쓰기FORM)
...
COMMIT;
DELETE FROM BOARD WHERE BID=5 AND BPW='111';
ROLLBACK;
SELECT * FROM BOARD;